...
my $outputFilename = 'textheblex.tei.xml'; 

# Create output file
open (OUTF,">$outputFilename") or die "Could not open file textheblex.tei.xml for writing.";
...
#	$line =~ s/\\cf\s*(.+)/<ref target=\"Strong:$1\">$1<\/ref>/g;
	$line =~ s/\\cf\s*(.+)\n//g;
	$line =~ s/\\scr\s*([^\s]+)\.(\d+)\.(\d+)/<xr type="Bible"><ref osisRef="$1\.$2\.$3">$1 $2:$3<\/ref><\/xr>/g;
